...
class TabMenu extends StatelessWidget {
  ...
  final List<MenuInWeekDataModel> dataMenu;

  @override
  Widget build(BuildContext context) {
    final listTab = List.generate((dataMenu).length, (index) {
      return TabDayOfWeek(
        date: dataMenu[index].thucDonTuanNgay!.substring(0, 5),
        dayOfW: 'Thứ ${index + 2}',
      );
    });
  ...
  List<Widget> tabView(int index, BuildContext context) {
    return List.generate(dataMenu[index].thucDonTuanDataInWeek!.length,
        (innerIndex) {
      final menu = dataMenu[index].thucDonTuanDataInWeek![innerIndex];
